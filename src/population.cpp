...
Population::Population(std::vector<Genome*> genomeList, float power) {
	...
	for (std::vector<Genome*>::iterator iter = genomeList.begin();
			iter != genomeList.end(); ++iter) {
...
Population::~Population() {
	...
	for (std::vector<Innovation*>::iterator iter = innovations.begin();
			iter != innovations.end(); ++iter)
		delete *iter;
...
bool Population::print_to_file_by_species(char *filename) {
	...
	for (curspecies = species.begin(); curspecies != species.end();
			++curspecies) {
...
bool Population::epoch(int generation) {
	...
	std::cout << "Generation " << generation << ": " << "overall_average = "
			<< overall_average << std::endl;

	//Now compute expected number of offspring for each individual organism
	for (curorg = organisms.begin(); curorg != organisms.end(); ++curorg) {
		(*curorg)->expected_offspring =
				(((*curorg)->fitness) / overall_average);
	}
	...
	for (curspecies = sorted_species.begin();
		...
		std::cout << "orig fitness of Species" << (*curspecies)->id << "(Size "
				<< (*curspecies)->organisms.size() << "): "
				<< (*((*curspecies)->organisms).begin())->orig_fitness
				<< " last improved "
	...
	if (((*(((*curspecies)->organisms).begin()))->orig_fitness)
		...
		highest_fitness =
				((*(((*curspecies)->organisms).begin()))->orig_fitness);
		highest_last_changed = 0;
		std::cout << "NEW POPULATION RECORD FITNESS: " << highest_fitness
				<< std::endl;
	} else {
		++highest_last_changed;
		std::cout << highest_last_changed
				<< " generations since last population fitness record: "
				<< highest_fitness << std::endl;
	}
	...
	if (highest_last_changed >= NEAT::dropoff_age + 5) {
		...
		(*(((*curspecies)->organisms).begin()))->super_champ_offspring =
				half_pop;
	...
	//  the system parameter babies_stolen (when babies_stolen > 0)
		...
		while ((stolen_babies < NUM_STOLEN)
			...
			if ((((*curspecies)->age) > 5)
				...
				if (((*curspecies)->expected_offspring - 1)
						>= (NUM_STOLEN - stolen_babies)) {
		...
		while ((curspecies != sorted_species.end())
				&& ((*curspecies)->last_improved() > NEAT::dropoff_age))
			++curspecies;
