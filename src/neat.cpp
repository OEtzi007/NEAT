...
double NEAT::trait_param_mut_prob = 0; //Prob that a trait parameter gets mutated
double NEAT::trait_mutation_power = 0; // Power (absolute value change) of mutation on a signle trait param
...
double NEAT::oldhebbian(double weight, double maxweight, double active_in,
	...
	if (!(neg)) {
		...
		//input is high
		delta = hebb_rate * (maxweight - weight) * active_in
				* (1.0 - active_out) + //"unhebb"
...
double NEAT::hebbian(double weight, double maxweight, double active_in,
		double active_out, double hebb_rate, double pre_rate,
		double post_rate) {
